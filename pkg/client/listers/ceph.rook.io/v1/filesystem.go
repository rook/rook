...
type FilesystemLister interface {
	...
	List(selector labels.Selector) (ret []*v1.Filesystem, err error)
...
// List lists all Filesystems in the indexer.
func (s *filesystemLister) List(selector labels.Selector) (ret []*v1.Filesystem, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.Filesystem))
	})
	return ret, err
}
...
type FilesystemNamespaceLister interface {
	...
	Get(name string) (*v1.Filesystem, error)
...
// List lists all Filesystems in the indexer for a given namespace.
func (s filesystemNamespaceLister) List(selector labels.Selector) (ret []*v1.Filesystem, err error) {
...
// Get retrieves the Filesystem from the indexer for a given namespace and name.
func (s filesystemNamespaceLister) Get(name string) (*v1.Filesystem, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("filesystem"), name)
	}
	return obj.(*v1.Filesystem), nil
}
