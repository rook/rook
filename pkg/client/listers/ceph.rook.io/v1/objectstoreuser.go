...
type ObjectStoreUserLister interface {
	...
	List(selector labels.Selector) (ret []*v1.ObjectStoreUser, err error)
...
// List lists all ObjectStoreUsers in the indexer.
func (s *objectStoreUserLister) List(selector labels.Selector) (ret []*v1.ObjectStoreUser, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.ObjectStoreUser))
	})
	return ret, err
}
...
type ObjectStoreUserNamespaceLister interface {
	...
	Get(name string) (*v1.ObjectStoreUser, error)
...
// List lists all ObjectStoreUsers in the indexer for a given namespace.
func (s objectStoreUserNamespaceLister) List(selector labels.Selector) (ret []*v1.ObjectStoreUser, err error) {
...
// Get retrieves the ObjectStoreUser from the indexer for a given namespace and name.
func (s objectStoreUserNamespaceLister) Get(name string) (*v1.ObjectStoreUser, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("objectstoreuser"), name)
	}
	return obj.(*v1.ObjectStoreUser), nil
}
