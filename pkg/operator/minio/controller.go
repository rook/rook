...
func (c *MinioController) buildMinioCtrArgs(statefulSetPrefix, headlessServiceName, namespace string, serverCount int32, volumeClaims []v1.PersistentVolumeClaim) []string {
	args := []string{"server"}
	for i := int32(0); i < serverCount; i++ {
		for i := range volumeClaims {
			serverAddress := fmt.Sprintf("http://%s-%d.%s.%s%s", statefulSetPrefix, i, headlessServiceName, namespace, getPVCDataDir(volumeClaims[i]))
...
func (c *MinioController) makeMinioPodSpec(name, namespace string, ctrName string, ctrImage string, port int32, envVars map[string]string, numServers int32, volumeClaims []v1.PersistentVolumeClaim) v1.PodTemplateSpec {
	var env []v1.EnvVar
	for k, v := range envVars {
		env = append(env, v1.EnvVar{Name: k, Value: v})
	}

	volumeMounts := make([]v1.VolumeMount, len(volumeClaims))
	for i := range volumeClaims {
		volumeMounts = append(volumeMounts, v1.VolumeMount{
			Name:      volumeClaims[i].GetName(),
			MountPath: getPVCDataDir(volumeClaims[i]),
		})
	}
			...
			Containers: []v1.Container{
				{
					...
					Args:         c.buildMinioCtrArgs(name, name, namespace, numServers, volumeClaims),
...
func (c *MinioController) makeMinioStatefulSet(name, namespace string, spec miniov1alpha1.ObjectStoreSpec, ownerRef meta_v1.OwnerReference) (*v1beta2.StatefulSet, error) {
	...
	podSpec := c.makeMinioPodSpec(name, namespace, minioCtrName, c.rookImage, spec.Port, envVars, int32(spec.Storage.NodeCount), spec.Storage.VolumeClaimTemplates)
...
func getPVCDataDir(pvc v1.PersistentVolumeClaim) string {
	return fmt.Sprintf(objectStoreDataDirTemplate, pvc.GetName())
}
