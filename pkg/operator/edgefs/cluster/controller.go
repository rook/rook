...
var ClusterResource = opkit.CustomResource{
	...
	Kind:    reflect.TypeOf(edgefsv1alpha1.Cluster{}).Name(),
}
...
func (c *ClusterController) StartWatch(namespace string, stopCh chan struct{}) error {
	...
	watcher := opkit.NewWatcher(ClusterResource, namespace, resourceHandlerFuncs, c.context.RookClientset.EdgefsV1alpha1().RESTClient())
	go watcher.Watch(&edgefsv1alpha1.Cluster{}, stopCh)
...
func (c *ClusterController) onAdd(obj interface{}) {
	clusterObj := obj.(*edgefsv1alpha1.Cluster).DeepCopy()
	...
	if c.devicesInUse && cluster.Spec.Storage.AnyUseAllDevices() {
		...
		if err := c.updateClusterStatus(clusterObj.Namespace, clusterObj.Name, edgefsv1alpha1.ClusterStateError, message); err != nil {
	...
	err := wait.Poll(clusterCreateInterval, clusterCreateTimeout, func() (bool, error) {
		if err := c.updateClusterStatus(clusterObj.Namespace, clusterObj.Name, edgefsv1alpha1.ClusterStateCreating, ""); err != nil {
			logger.Errorf("failed to update cluster status in namespace %s: %+v", cluster.Namespace, err)
			return false, nil
		}
		...
		if err := c.updateClusterStatus(clusterObj.Namespace, clusterObj.Name, edgefsv1alpha1.ClusterStateCreated, ""); err != nil {
			logger.Errorf("failed to update cluster status in namespace %s: %+v", cluster.Namespace, err)
			return false, nil
		}
...
func (c *ClusterController) onUpdate(oldObj, newObj interface{}) {
	oldCluster := oldObj.(*edgefsv1alpha1.Cluster).DeepCopy()
	newCluster := newObj.(*edgefsv1alpha1.Cluster).DeepCopy()
	...
	if err != nil {
		...
		logger.Error(message)
		if err := c.updateClusterStatus(newCluster.Namespace, newCluster.Name, edgefsv1alpha1.ClusterStateError, message); err != nil {
...
func (c *ClusterController) handleUpdate(newClust *edgefsv1alpha1.Cluster, cluster *cluster) (bool, error) {
	if err := c.updateClusterStatus(newClust.Namespace, newClust.Name, edgefsv1alpha1.ClusterStateUpdating, ""); err != nil {
	...
	if err := c.updateClusterStatus(newClust.Namespace, newClust.Name, edgefsv1alpha1.ClusterStateCreated, ""); err != nil {
		logger.Errorf("failed to update cluster status in namespace %s: %+v", newClust.Namespace, err)
		return false, nil
	}
...
func (c *ClusterController) onDelete(obj interface{}) {
	clust := obj.(*edgefsv1alpha1.Cluster).DeepCopy()
...
func (c *ClusterController) updateClusterStatus(namespace, name string, state edgefsv1alpha1.ClusterState, message string) error {
	// get the most recent cluster CRD object
	cluster, err := c.context.RookClientset.EdgefsV1alpha1().Clusters(namespace).Get(name, metav1.GetOptions{})
	if err != nil {
	...
	cluster.Status = edgefsv1alpha1.ClusterStatus{State: state, Message: message}
	if _, err := c.context.RookClientset.EdgefsV1alpha1().Clusters(cluster.Namespace).Update(cluster); err != nil {
...
func (c *ClusterController) initCluster(cluster *cluster) error {
	if cluster.spec.Storage.NodeCount < 1 {
		logger.Infof("NodeCount should be > 0")
		return nil
	}

	logger.Infof("Edgefs cluster init succeeded for namespace %s", cluster.namespace)
	return nil
}
*/

func (c *ClusterController) addFinalizer(clust *edgefsv1alpha1.Cluster) error {

	// get the latest cluster object since we probably updated it before we got to this point (e.g. by updating its status)
	clust, err := c.context.RookClientset.EdgefsV1alpha1().Clusters(clust.Namespace).Get(clust.Name, metav1.GetOptions{})
	if err != nil {
		return err
	}
	...
	// update the crd
	_, err = c.context.RookClientset.EdgefsV1alpha1().Clusters(clust.Namespace).Update(clust)
	if err != nil {
...
func (c *ClusterController) removeFinalizer(obj interface{}) {
	...
	if cl, ok := obj.(*edgefsv1alpha1.Cluster); ok {
		fname = finalizerName
	...
	for i := 0; i < maxRetries; i++ {
		var err error
		if cluster, ok := obj.(*edgefsv1alpha1.Cluster); ok {
			_, err = c.context.RookClientset.EdgefsV1alpha1().Clusters(cluster.Namespace).Update(cluster)
		}
