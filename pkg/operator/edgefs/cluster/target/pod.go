...
func (c *Cluster) makeCorosyncContainer(containerImage string) v1.Container {
	...
	return v1.Container{
		...
		Args:            []string{"edgefs", "start", "corosync"},
...
func (c *Cluster) makeAuditdContainer(containerImage string) v1.Container {
	...
	return v1.Container{
		...
		Image:           containerImage,
		ImagePullPolicy: v1.PullAlways,
		Args:            []string{"edgefs", "start", "auditd"},
		Env: []v1.EnvVar{
			{
				Name:  "CCOW_LOG_LEVEL",
				Value: "5",
...
func (c *Cluster) makeDaemonContainer(containerImage string, zap bool) v1.Container {
	...
	name := "daemon"
	args := []string{"edgefs", "start", "daemon"}
	if zap {
		name = "daemon-zap"
		args = []string{"edgefs", "start", "toolbox", "nezap --do-as-i-say"}
...
func (c *Cluster) createPodSpec(rookImage string, zap, resurrect bool) v1.PodSpec {
	...
	if zap {
		...
		initContainers = []v1.Container{
			c.makeDaemonContainer(rookImage, true),
		}
	}
	...
	containers = []v1.Container{
		...
		c.makeDaemonContainer(rookImage, false),
	}
...
func (c *Cluster) makeStatefulSet(replicas int32, rookImage string, zap, resurrect bool) (*appsv1beta1.StatefulSet, error) {
	...
	statefulSet := &appsv1beta1.StatefulSet{
		...
		Spec: appsv1beta1.StatefulSetSpec{
			...
			Template: v1.PodTemplateSpec{
				...
				Spec: c.createPodSpec(rookImage, zap, resurrect),
			},
