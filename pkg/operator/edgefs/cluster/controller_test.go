...
func TestClusterDelete(t *testing.T) {
	...
	clusterToDelete := &edgefsv1alpha1.Cluster{ObjectMeta: metav1.ObjectMeta{Namespace: clusterName}}
...
func TestClusterChanged(t *testing.T) {
	...
	old := edgefsv1alpha1.ClusterSpec{
...
func TestRemoveFinalizer(t *testing.T) {
	...
	// *****************************************
	cluster := &edgefsv1alpha1.Cluster{
	...
	cluster, err := context.RookClientset.EdgefsV1alpha1().Clusters(cluster.Namespace).Create(cluster)
	...
	cluster, err = context.RookClientset.EdgefsV1alpha1().Clusters(cluster.Namespace).Get(cluster.Name, metav1.GetOptions{})
	assert.NoError(t, err)
