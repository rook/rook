...
func (c *cluster) createInstance(rookImage string) error {
	...
	c.targets = target.New(c.context, c.Namespace, "latest", c.Spec.ServiceAccount, c.Spec.Storage, c.Spec.DataDirHostPath, c.Spec.DataVolumeSize,
		edgefsv1alpha1.GetTargetPlacement(c.Spec.Placement), c.Spec.Network.HostNetwork, edgefsv1alpha1.GetTargetResources(c.Spec.Resources), c.ownerRef)

	err = c.targets.Start(rookImage, c.Spec.ZapDevices, c.Spec.Resurrect)
	if err != nil {
	...
	c.mgrs = mgr.New(c.context, c.Namespace, "latest", c.Spec.ServiceAccount, c.Spec.DataDirHostPath, c.Spec.DataVolumeSize,
		edgefsv1alpha1.GetMgrPlacement(c.Spec.Placement), c.Spec.Network.HostNetwork,
		edgefsv1alpha1.GetMgrResources(c.Spec.Resources), c.ownerRef)
