...
func TestStartMGR(t *testing.T) {
	...
	c := New(context, "ns", "myversion", "", "", *volSize, rookalpha.Placement{}, false, v1.ResourceRequirements{}, metav1.OwnerReference{})
...
func TestPodSpec(t *testing.T) {
	volSize := resource.NewQuantity(100000.0, resource.BinarySI)
	c := New(&clusterd.Context{Clientset: testop.New(1)}, "ns", "rook/rook:myversion", "", "", *volSize, rookalpha.Placement{}, false, v1.ResourceRequirements{
	...
	assert.Equal(t, "edgefs", cont.Args[0])
	assert.Equal(t, "start", cont.Args[1])
	assert.Equal(t, "mgmt", cont.Args[2])
...
func TestServiceSpec(t *testing.T) {
	volSize := resource.NewQuantity(100000.0, resource.BinarySI)
	c := New(&clusterd.Context{}, "ns", "myversion", "", "", *volSize, rookalpha.Placement{}, false, v1.ResourceRequirements{}, metav1.OwnerReference{})
...
func TestHostNetwork(t *testing.T) {
	volSize := resource.NewQuantity(100000.0, resource.BinarySI)
	c := New(&clusterd.Context{Clientset: testop.New(1)}, "ns", "myversion", "", "", *volSize, rookalpha.Placement{}, true, v1.ResourceRequirements{}, metav1.OwnerReference{})
