...
func (c *NFSController) CreateOrUpdate(s edgefsv1alpha1.NFS, update bool, ownerRefs []metav1.OwnerReference) error {
	...
	deployment := c.makeDeployment(s.Name, s.Namespace, c.rookImage, s.Spec.Instances)
...
func (c *NFSController) makeDeployment(svcname, namespace, rookImage string, replicas int32) *extensions.Deployment {
	...
	podSpec := v1.PodTemplateSpec{
		...
		Spec: v1.PodSpec{
			Containers:    []v1.Container{c.nfsContainer(svcname, name, rookImage)},
...
func (c *NFSController) nfsContainer(svcname, name, containerImage string) v1.Container {

	privileged := c.hostNetwork
	runAsUser := int64(0)
