...
func (r *ReconcileCephNFS) reconcile(request reconcile.Request) (reconcile.Result, cephv1.CephNFS, error) {
	...
	if !isReadyToReconcile {
		...
		if !cephNFS.GetDeletionTimestamp().IsZero() && !cephClusterExists {
			...
			r.recorder.Eventf(cephNFS, nil, v1.EventTypeNormal, string(cephv1.ReconcileSucceeded), "", "successfully removed finalizer")
	...
	if !cephNFS.GetDeletionTimestamp().IsZero() {
		...
		r.recorder.Eventf(cephNFS, nil, v1.EventTypeNormal, string(cephv1.ReconcileStarted), "", "deleting CephNFS %q", cephNFS.Name)
