...
func (r *ReconcileClusterDisruption) reconcilePDBsForOSDs(
	...
	maxUnavailableOSDCount := 0
	// below switch block will update the PDB state config map based on the PG status
	switch {
	...
	case osdDown && pgClean:
		...
		logger.Infof("OSD(s) %q down but PGs are clean. PG Status: %q", downOSDs, pgHealthMsg)
		resetPDBConfig(pdbStateMap)
	case osdDown && !pgClean:
		setPDBConfig(pdbStateMap, osdDownFailureDomains, nodeDrainFailureDomains)
		logger.Infof("OSD(s) down and PGs are not clean. PGs Status: %q", pgHealthMsg)
...
func (r *ReconcileClusterDisruption) getOSDFailureDomains(clusterInfo *cephclient.ClusterInfo, request reconcile.Request, poolFailureDomain string) ([]string, []string, []string, []string, error) {
	...
	for _, deployment := range osdDeploymentList.Items {
		...
		if deployment.Status.ReadyReplicas < 1 {
			...
			if !downOSDs.Has(labels[osd.OsdIdLabelKey]) {
				downOSDs.Insert(labels[osd.OsdIdLabelKey])
			}
			// check if OSD is down on unscheduleable node
			isDrained, err := hasOSDNodeDrained(clusterInfo.Context, r.client, labels["topology-location-host"])
			if err != nil {
	...
	return sets.List(allFailureDomains), sets.List(nodeDrainFailureDomains), sets.List(osdDownFailureDomains), sets.List(downOSDs), nil
}
...
// - allowedDisruption in main PDB is 0, that is, One or more OSD went down. Adds 30 second interval before requeue so that rook can fetch the correct PG status
// - MaxUnavailable in the main PDB is > 1, that is, OSDs are down but PGs are clean.
// - Blocking PDBs are created.
