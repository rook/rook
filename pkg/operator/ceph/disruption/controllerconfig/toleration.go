...
func (t *TolerationSet) ToList() []corev1.Toleration {
	...
	// TolerationSet uses the same key for deduplication, so no two entries can
	// compare equal and a stable sort is not required.
	slices.SortFunc(tolerationList, func(a, b corev1.Toleration) int {
