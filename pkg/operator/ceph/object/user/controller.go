...
func (r *ReconcileObjectStoreUser) reconcileUserKeys(nsName types.NamespacedName, targetKeys []admin.UserKeySpec) error {
	...
	for i, existingKey := range userInfo.Keys {
		...
		if !found {
			...
			log.NamedDebug(nsName, logger, "removed key %d from user %q as it is not in the target list", i, userID)
			continue
		}
		if existingKey.SecretKey != targetKey.SecretKey {
			...
			log.NamedDebug(nsName, logger, "removed key %d from user %q as it needs to be recreated", i, userID)
			continue
		}
	...
	for _, k := range targetMap {
		...
		log.NamedDebug(nsName, logger, "created a key for user %q", userID)
	}
