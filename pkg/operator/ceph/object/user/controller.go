...
func (r *ReconcileObjectStoreUser) reconcile(request reconcile.Request) (reconcile.Result, cephv1.CephObjectStoreUser, error) {
	...
	if err != nil {
		if !cephObjectStoreUser.GetDeletionTimestamp().IsZero() {
			...
			r.recorder.Eventf(cephObjectStoreUser, nil, corev1.EventTypeNormal, string(cephv1.ReconcileSucceeded), "", "successfully removed finalizer")
	...
	if !cephObjectStoreUser.GetDeletionTimestamp().IsZero() {
		...
		r.recorder.Eventf(cephObjectStoreUser, nil, corev1.EventTypeNormal, string(cephv1.ReconcileStarted), "", "deleting CephObjectStoreUser %q", cephObjectStoreUser.Name)
