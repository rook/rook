...
func TestVerifyLiveUserTenant(t *testing.T) {
	newUser := func(tenant string) *cephv1.CephObjectStoreUser {
		return &cephv1.CephObjectStoreUser{
			ObjectMeta: metav1.ObjectMeta{Name: "user1"},
			Spec:       cephv1.ObjectStoreUserSpec{Tenant: tenant},
		}
	}

	t.Run("matching untenanted user is valid", func(t *testing.T) {
		assert.NoError(t, verifyLiveUserTenant(newUser(""), &admin.User{ID: "user1"}))
	})

	t.Run("bare live ID for a tenanted user is valid", func(t *testing.T) {
		// Admin Ops returns the tenant in a separate field that go-ceph does not decode
		assert.NoError(t, verifyLiveUserTenant(newUser("tenantA"), &admin.User{ID: "user1"}))
	})

	t.Run("combined live ID in the matching tenant is valid", func(t *testing.T) {
		assert.NoError(t, verifyLiveUserTenant(newUser("tenantA"), &admin.User{ID: "tenantA$user1"}))
	})

	t.Run("combined live ID in a different tenant is rejected", func(t *testing.T) {
		assert.Error(t, verifyLiveUserTenant(newUser("tenantA"), &admin.User{ID: "tenantB$user1"}))
	})

	t.Run("combined live ID does not satisfy an untenanted spec", func(t *testing.T) {
		assert.Error(t, verifyLiveUserTenant(newUser(""), &admin.User{ID: "tenantA$user1"}))
	})

	t.Run("live user with a different name is rejected", func(t *testing.T) {
		assert.Error(t, verifyLiveUserTenant(newUser("tenantA"), &admin.User{ID: "user2"}))
	})
}
...
func TestCreateOrUpdateCephUserWithTenant(t *testing.T) {
	...
	//nolint:gosec // only test values, not a real secret
	// matches the Admin Ops response shape: bare user_id, tenant reported separately
	tenantUserJSON := `{
	"tenant": "tenantA",
	"user_id": "user1",
	"display_name": "user1",
	...
	// the mock only answers requests addressed as "tenantA$user1"; the response
	// itself carries the bare name, as the Admin Ops API does
	liveUser, err := r.createOrUpdateCephUser(objectUser, userConfig)
	require.NoError(t, err)
	assert.Equal(t, "user1", liveUser.ID)
}
