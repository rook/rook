...
func createTopicAttributes(p provisioner, topic *cephv1.CephBucketTopic) (map[string]*string, *map[types.UID]*corev1.Secret, error) {
	...
	if topic.Spec.Endpoint.Kafka != nil {
		...
		// If UserRef or PassRef is set, we need to parse the URI and insert the
		// credentials as http basic auth. If basic auth was already set as part of
		// the URI string, it will be overridden.
		if kafka.UserRef != nil || kafka.PassRef != nil {
			var user, pass string

			if kafka.UserRef != nil {
				var secret *corev1.Secret
				user, secret, err = p.getSecretValue(kafka.UserRef, topic.Namespace)
				if err != nil {
					return nil, nil, errors.Wrapf(err, "failed to get secret value from CephBucketTopic %q .spec.endpoint.kafka.userRef %q", nsName, kafka.UserRef)
				}
			...
			if kafka.PassRef != nil {
				var secret *corev1.Secret
				pass, secret, err = p.getSecretValue(kafka.PassRef, topic.Namespace)
				if err != nil {
					return nil, nil, errors.Wrapf(err, "failed to get secret value from CephBucketTopic %q .spec.endpoint.kafka.passRef %q", nsName, kafka.PassRef)
				}
				logger.Debugf("CephBucketTopic %q references secret %q", nsName, client.ObjectKeyFromObject(secret))
				referencedSecrets[secret.UID] = secret
			}
