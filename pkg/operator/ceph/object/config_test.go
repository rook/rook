...
func TestRgwFrontendStr(t *testing.T) {
	...
	// With tls_groups only (default ssl_options still applied)
	cfg = newConfig(t)
	cfg.clusterSpec.Network.HostNetwork = true
	cfg.store.Spec.Gateway.Port = 80
	cfg.store.Spec.Security = &cephv1.ObjectStoreSecuritySpec{
	...
	result = cfg.rgwFrontendStr()
	assert.Equal(t, "beast port=80 ssl_options=no_sslv2:no_sslv3:no_tlsv1:no_tlsv1_1 tls_groups=[X25519 P-256]", result, "case 6")
}
