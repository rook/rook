...
func TestCephObjectStoreControllerZoneNotReady(t *testing.T) {
	...
	for _, tc := range zoneNotReadyPhases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Reconcile should fail because the zone is not ready.
			// The non-nil error causes controller-runtime to requeue with backoff,
			// preventing RGW from starting before zone shared pools are configured.
			assert.Error(t, err)
