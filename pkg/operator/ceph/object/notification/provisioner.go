...
var createNotification = func(p provisioner, bucket *bktv1alpha1.ObjectBucket, topicARN string, notification *cephv1.CephBucketNotification) error {
	...
	_, err = s3Agent.ClientV2.PutBucketNotificationConfiguration(context.TODO(), &s3v2.PutBucketNotificationConfigurationInput{
		Bucket: &bucketName,
...
var getAllRGWNotifications = func(p provisioner, ob *bktv1alpha1.ObjectBucket) ([]string, error) {
	...
	nc, err := s3Agent.ClientV2.GetBucketNotificationConfiguration(context.TODO(), &s3v2.GetBucketNotificationConfigurationInput{
		Bucket:              &bucketName,
