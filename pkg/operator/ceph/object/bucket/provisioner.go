...
func (p *Provisioner) setBucketPolicy(bucket *bucket) error {
	...
	policyResp, err := svc.GetBucketPolicy(context.TODO(), &s3v2.GetBucketPolicyInput{
		Bucket: &p.bucketName,
	})
	if err != nil {
		var apiErr smithy.APIError
	...
	if additionalConfig.bucketPolicy == nil {
		_, err = svc.DeleteBucketPolicy(context.TODO(), &s3v2.DeleteBucketPolicyInput{
			Bucket: &p.bucketName,
		})
		if err != nil {
		...
		_, err = svc.PutBucketPolicy(context.TODO(), &s3v2.PutBucketPolicyInput{
			Bucket: &p.bucketName,
...
func (p *Provisioner) setBucketLifecycle(bucket *bucket) error {
	...
	liveLc, err := svc.GetBucketLifecycleConfiguration(context.TODO(), &s3v2.GetBucketLifecycleConfigurationInput{
		Bucket: &p.bucketName,
	})
	if err != nil {
		var apiErr smithy.APIError
	...
	// cmpopts.IgnoreUnexported is required because v2 SDK types embed an
	// unexported noSmithyDocumentSerde field that cmp.Diff cannot handle.
	...
	if additionalConfig.bucketLifecycle == nil {
		_, err = svc.DeleteBucketLifecycle(context.TODO(), &s3v2.DeleteBucketLifecycleInput{
			Bucket: &p.bucketName,
		})
		if err != nil {
		...
		_, err = svc.PutBucketLifecycleConfiguration(context.TODO(), &s3v2.PutBucketLifecycleConfigurationInput{
			Bucket:                 &p.bucketName,
