	...
	return predicate.Funcs{
		...
		UpdateFunc: func(e event.UpdateEvent) bool {
			...
			case *cephv1.CephObjectStore:
				...
				if objOld.Spec.Gateway.ExternalRgwEndpoints[0] != objNew.Spec.Gateway.ExternalRgwEndpoints[0] {
					logger.Infof("there is an update event on CephObjectStore CR, so reconcile the bucket provisioner")
