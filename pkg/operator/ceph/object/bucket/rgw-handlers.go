...
func random(fsid string) string {
	randString := ""
	for range fsid {
		rand, _ := rand.Int(rand.Reader, big.NewInt(int64(len(fsid))))
		randString += string(fsid[rand.Int64()])
	}
...
func (p *Provisioner) genUserName(obcName, obcNamespace string) (string, error) {
	...
	// However, it can also collide with external clusters, so use random
	fsid := p.clusterInfo.FSID
	if p.clusterInfo.FSID == "" {
		return "", errors.Errorf("failed to find ceph cluster FSID")
	}
	return "obc-" + obcNamespace + "-" + obcName + "-" + random(fsid), nil
}
