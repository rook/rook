...
func (r *ReconcileObjectStoreAccount) reconcile(request reconcile.Request) (reconcile.Result, cephv1.CephObjectStoreAccount, error) {
	...
	if err != nil {
		if !cephObjectStoreAccount.GetDeletionTimestamp().IsZero() {
			...
			r.recorder.Event(cephObjectStoreAccount, corev1.EventTypeNormal, string(cephv1.ReconcileSucceeded), "successfully removed finalizer")
	...
	if !cephObjectStoreAccount.GetDeletionTimestamp().IsZero() {
		...
		r.recorder.Eventf(cephObjectStoreAccount, corev1.EventTypeNormal, string(cephv1.ReconcileStarted), "deleting CephObjectStoreAccount %q", cephObjectStoreAccount.Name)
	...
	r.recorder.Eventf(cephObjectStoreAccount, corev1.EventTypeNormal, string(cephv1.ReconcileSucceeded), "CephObjectStoreAccount %q reconciled successfully", cephObjectStoreAccount.Name)
...
func (r *ReconcileObjectStoreAccount) reconcileAccount(cephObjectStoreAccount *cephv1.CephObjectStoreAccount) (string, error) {
	...
	if err != nil {
		...
		if strings.Contains(err.Error(), "already exists") || strings.Contains(err.Error(), "EntityExists") {
