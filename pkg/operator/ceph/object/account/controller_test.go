...
func TestDeleteAccount(t *testing.T) {
	...
	t.Run("delete account with annotation ownership proof", func(t *testing.T) {
		deleteCalled := false
		mockClient := &cephobject.MockClient{
			MockDo: func(req *http.Request) (*http.Response, error) {
				if req.Method == http.MethodDelete {
		...
		// spec.AccountID is set and annotation confirms ownership
		account := &cephv1.CephObjectStoreAccount{
			ObjectMeta: metav1.ObjectMeta{
				Name:        name,
				Namespace:   namespace,
		...
		err = r.deleteAccount(account)
		assert.NoError(t, err)
		assert.True(t, deleteCalled, "should call RGW delete when annotation confirms ownership")
	})
}
