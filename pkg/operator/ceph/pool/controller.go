...
func (r *ReconcileCephBlockPool) reconcile(request reconcile.Request) (reconcile.Result, cephv1.CephBlockPool, error) {
	...
	if cephBlockPool.Status == nil {
		updateStatus(r.opManagerContext, r.client, request.NamespacedName, cephv1.ConditionProgressing, buildStatusInfo(cephBlockPool), k8sutil.ObservedGenerationNotAvailable)
	}
	...
	if err != nil {
		...
		updateStatus(r.opManagerContext, r.client, request.NamespacedName, cephv1.ConditionFailure, buildStatusInfo(cephBlockPool), k8sutil.ObservedGenerationNotAvailable)
	...
	if cephBlockPool.Spec.Mirroring.Enabled {
		...
		// update ObservedGeneration in status at the end of reconcile
		// Set Ready status, we are done reconciling
		updateStatus(r.opManagerContext, r.client, request.NamespacedName, cephv1.ConditionReady, buildStatusInfo(cephBlockPool), observedGeneration)
