...
func (r *ReconcileCephClient) reconcile(request reconcile.Request) (reconcile.Result, cephv1.CephClient, error) {
	...
	if !cephClient.GetDeletionTimestamp().IsZero() {
		...
		r.recorder.Eventf(cephClient, nil, v1.EventTypeNormal, string(cephv1.ReconcileStarted), "", "deleting CephClient %q", cephClient.Name)
		...
		r.recorder.Eventf(cephClient, nil, v1.EventTypeNormal, string(cephv1.ReconcileSucceeded), "", "successfully removed finalizer")
