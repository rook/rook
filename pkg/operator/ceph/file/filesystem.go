...
func createFilesystem(
	...
	allowStandbyReplay := false
	standbyCountWanted := fs.Spec.MetadataServer.StandbyCount
	if fs.Spec.MetadataServer.StandbyCount == 0 && fs.Spec.MetadataServer.ActiveStandby {
		allowStandbyReplay = true
		standbyCountWanted = fs.Spec.MetadataServer.ActiveCount
	}

	if err := cephclient.SetAllowStandbyReplay(context, clusterInfo, fs.Name, allowStandbyReplay); err != nil {
		return errors.Wrapf(err, "failed to set allow_standby_replay to filesystem %q", fs.Name)
	}
	if err := cephclient.SetStandbyCountWanted(context, clusterInfo, fs.Name, standbyCountWanted); err != nil {
