...
func ReportReconcileResult(
	...
	if err != nil {
		...
		// 2. event
		recorder.Eventf(objCopy, nil, corev1.EventTypeWarning, string(cephv1.ReconcileFailed), "", errorMsg)
		...
		logger.Debug(msg)
		recorder.Eventf(objCopy, nil, corev1.EventTypeNormal, string(cephv1.ReconcileRequeuing), "", msg)
	} else {
		...
		// 2. event
		recorder.Eventf(objCopy, nil, corev1.EventTypeNormal, string(cephv1.ReconcileSucceeded), "", successMsg)
	}
...
func ReportDeletionNotBlockedDueToDependents(
	...
	// 2. event
	recorder.Eventf(obj, nil, corev1.EventTypeNormal, string(cephv1.DeletingReason), "", deletingMsg)
