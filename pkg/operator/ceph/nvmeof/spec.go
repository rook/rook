...
func (r *ReconcileCephNVMeOFGateway) connectionConfigInitContainer(nvmeof *cephv1.CephNVMeOFGateway, configMapName string) v1.Container {
	...
	if poolName == "" {
		logger.Debug("connectionConfigInitContainer() pool name not specified, using default: nvmeofpool")
		poolName = "nvmeofpool"
	} else {
		logger.Debugf("connectionConfigInitContainer() using specified pool: %s", poolName)
	}
