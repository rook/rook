...
func (c *createConfig) createNewOSDsFromStatus(
	...
	for i, osd := range status.OSDs {
		...
		osd.CephxStatus = keyring.UpdatedCephxStatus(false, c.cluster.spec.Security.CephX.Daemon,
			c.cluster.clusterInfo.CephVersion, keyring.UninitializedCephxStatus())
		logger.Debugf("creating new OSD deployment for OSD.%d with cephx status %#v", osd.ID, osd.CephxStatus) // ===== TESTING TESTING TESTING TESTING =====
