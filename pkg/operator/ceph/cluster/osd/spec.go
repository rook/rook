...
func (c *Cluster) makeDeployment(osdProps osdProperties, osd *OSDInfo, provisionConfig *provisionConfig) (*apps.Deployment, error) {
	...
	var command []string
	var args []string
	// mainContainerRunsRookBinary is set when the osd daemon container itself execs the rook
	// binary (LVM-on-PVC), which is the only topology whose main container needs the rook-binaries
	// mount. Node-based OSDs run the rook binary only in the activate init container.
	mainContainerRunsRookBinary := false
	...
	if osdProps.onPVC() {
		if osd.CVMode == "lvm" {
			...
			mainContainerRunsRookBinary = true
			args = []string{
				"ceph", "osd", "start",
				"--",
			}
		...
		// non-pvc: the binary copy stays, the activate init container runs the rook binary
		doConfigInit = false
		command = []string{"ceph-osd"}
	}
	...
	if doBinaryCopyInit {
		// match the OSD resources so the copy-bins container does not demote the pod's QoS class
		copyBinariesContainer.Resources = osdProps.resources
		volumes = append(volumes, copyBinariesVolume)
		if mainContainerRunsRookBinary {
...
func (c *Cluster) getActivateOSDInitContainer(configDir, namespace, osdID string, osdInfo OSDInfo, osdProps osdProperties) ([]v1.Volume, *v1.Container) {
	...
	container := &v1.Container{
		...
		Args: []string{
			...
			"--osd-store-flag", osdStoreFlag,
			"--cv-mode", osdInfo.CVMode,
			"--block-path", osdInfo.BlockPath,
			fmt.Sprintf("--encrypted-device=%t", osdProps.storeConfig.EncryptedDevice),
		},
		Name:            "activate",
		Image:           c.spec.CephVersion.Image,
		ImagePullPolicy: controller.GetContainerImagePullPolicy(c.spec.CephVersion.ImagePullPolicy),
