...
# TODO: can delete?
# always store the latest keyring file, in case key generation during OSD prepare failed, or in case of OSD key rotation
# ceph -n client.admin auth get-or-create osd."$OSD_ID" \
#   mon 'allow profile osd' mgr 'allow profile osd' osd 'allow *' \
#   -k /etc/ceph/admin-keyring-store/keyring > "$KEYRING_FILE"
...
ls /etc/ceph/admin-keyring-store
cat /etc/ceph/admin-keyring-store/keyring
