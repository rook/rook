...
func TestCheckTopologyConflicts(t *testing.T) {
	...
	// full 9‑level hierarchy, identical across two nodes → valid
	...
	// region only → valid (tests that missing deeper levels are skipped)
	...
	// zone→datacenter count mismatch (2 zones but only 1 datacenter) → invalid
	...
	// rack only with two distinct chassis → valid (deep child > parent)
	...
	// same chassis under two different racks → invalid (child count < parent count)
	...
	// cross‑key duplication: same value under pod and rack → invalid
	...
	// invalid: fewer rooms than datacenters (datacenter=2, room=1)
	t.Run("invalid: fewer rooms than datacenters", func(t *testing.T) {
		nodes := []corev1.Node{
		...
		assert.Error(t, err)
		assert.Contains(t, err.Error(), "topology.rook.io/datacenter")
		assert.Contains(t, err.Error(), "topology.rook.io/room")
	})

	// invalid: pod→pdu mismatch (2 pods but only 1 pdu)
	t.Run("invalid: fewer pdus than pods", func(t *testing.T) {
		nodes := []corev1.Node{
			node("a", map[string]string{
				"topology.rook.io/pod": "pod1",
				"topology.rook.io/pdu": "pdu1",
		...
		assert.Error(t, err)
		assert.Contains(t, err.Error(), "topology.rook.io/pod")
		assert.Contains(t, err.Error(), "topology.rook.io/pdu")
	})

	// valid: only pdu labels used (no parent “pod” at all)
	...
	// valid: hostname reuse in zone (hostname is ignored in hierarchy)
	...
	// invalid: non‑adjacent cross‑key duplicate (region vs rack)
	...
	// valid: mix zone/rack on some nodes and datacenter/room on others (disjoint)
	...
	// invalid: deeper count mismatch across non‑adjacent (datacenter=1, chassis=2 but region=2)
	t.Run("invalid: chassis too many under fewer regions", func(t *testing.T) {
		nodes := []corev1.Node{
			node("a", map[string]string{"topology.kubernetes.io/region": "R1", "topology.rook.io/chassis": "C1"}),
			node("b", map[string]string{"topology.kubernetes.io/region": "R2", "topology.rook.io/chassis": "C2"}),
			node("c", map[string]string{"topology.rook.io/chassis": "C3"}), // no region
		}
		err := CheckTopologyConflicts(&nodes)
		// regionCount=1, chassisCount=3 → childCount>0 && 3<1 false → valid
		assert.NoError(t, err)
	})
	t.Run("invalid: fewer rooms than datacenters", func(t *testing.T) {
		nodes := []corev1.Node{
