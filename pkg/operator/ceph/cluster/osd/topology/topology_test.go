...
func TestCheckTopologyConflicts(t *testing.T) {
	...
	t.Run("valid: racks only defined under one of multiple zones", func(t *testing.T) {
		nodes := []corev1.Node{
			node("rack-node-a", map[string]string{
				"topology.kubernetes.io/zone": "zone1",
				"topology.rook.io/rack":       "rack1",
			}),
			node("rack-node-b", map[string]string{
				"topology.kubernetes.io/zone": "zone1",
				"topology.rook.io/rack":       "rack2",
			}),
			node("rack-node-c", map[string]string{
				"topology.kubernetes.io/zone": "zone1",
				"topology.rook.io/rack":       "rack3",
			}),
			node("app-node-d", map[string]string{
				"topology.kubernetes.io/zone": "zone2",
			}),
			node("app-node-e", map[string]string{
				"topology.kubernetes.io/zone": "zone3",
			}),
			node("app-node-f", map[string]string{
