...
func TestGetOSDInfo(t *testing.T) {
	...
	t.Run("get info from node-based OSDs", func(t *testing.T) {
		...
		// simulate a deployment generated by a rook version whose activate init container was a
		// shell script carrying the block path in a DEVICE= assignment
		for i, cont := range d4.Spec.Template.Spec.InitContainers {
			if cont.Name == "activate" {
				d4.Spec.Template.Spec.InitContainers[i].Command = []string{"/bin/bash", "-c", "\nset -e\nDEVICE=/dev/vg1/lv1\n"}
				d4.Spec.Template.Spec.InitContainers[i].Args = nil
			}
		}
		osdInfo4, _ := c.getOSDInfo(d4)
		assert.Equal(t, osd4.ID, osdInfo4.ID)
		assert.Equal(t, location, osdInfo4.Location)
		assert.Equal(t, "/dev/vg1/lv1", osdInfo4.BlockPath)
