...
const (
	// OSDMigrationCofirmation is the confirmation provided by the user in the cephCluster spec.
	OSDMigrationCofirmation = "yes-really-migrate-osds"
...
func (c *Cluster) newMigrationConfig() (*migrationConfig, error) {
	...
	err = mc.migrateForStoreType(c, osdDeployments)
	if err != nil {
...
// migrateForStoreType gets all the OSDs that require migration due to change in the cephCluster OSD storeType setting
func (m *migrationConfig) migrateForStoreType(c *Cluster, osdDeployments *appsv1.DeploymentList) error {
	for i := range osdDeployments.Items {
