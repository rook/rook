...
func (c *Cluster) floatingMonInitContainer(podSpec *corev1.PodSpec, monConfig *monConfig) error {
	...
	err = yaml.Unmarshal([]byte(cm.Data["init-containers.yaml"]), &initContainers)
	if err != nil {
		return errors.Wrapf(err, "failed to unmarshal floating mon init containers from configmap %q", floatingMonCm)
	}
	...
	err = yaml.Unmarshal([]byte(cm.Data["volume-mounts.yaml"]), &volumeMounts)
	if err != nil {
		return errors.Wrapf(err, "failed to unmarshal floating mon volume mounts from configmap %q", floatingMonCm)
	}

	podSpec.InitContainers = append(initContainers, podSpec.InitContainers...)
	podSpec.Volumes = append(podSpec.Volumes, volumes...)
	// Add volume mounts to the mon daemon container which is the first container
