...
func (c *Cluster) RotateMonCephxKeys() (bool, error) {
	shouldRotateMonKeys, err := c.shouldRotateMonKeys()
	if err != nil {
		return false, errors.Wrapf(err, "failed to check if cephx keys for mon daemons in the namespace %q should be rotated", c.ClusterInfo.Namespace)
	}
	...
	err = controller.UpdateClusterAccessSecret(c.context.Clientset, c.ClusterInfo, c.ownerInfo)
	if err != nil {
...
func (c *Cluster) shouldRotateMonKeys() (bool, error) {
	...
	if err := c.context.Client.Get(c.ClusterInfo.Context, c.ClusterInfo.NamespacedName(), clusterObj); err != nil {
		return false, errors.Wrapf(err, "failed to get cluster %v.", c.ClusterInfo.NamespacedName())
	}
	...
	shouldRotateKeys, err := keyring.ShouldRotateCephxKeys(clusterObj.Spec.Security.CephX.Daemon, runningCephVersion, desiredCephVersion, *clusterObj.Status.Cephx.Mon)
	if err != nil {
		return false, errors.Wrap(err, "failed to check if mon daemon keys should be rotated or not")
	}
