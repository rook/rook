...
func (c *Cluster) isMetricsTLSEnabled() bool {
	return c.spec.Monitoring.MetricsTLS.SecretName != "" && !c.spec.External.Enable
}

func (c *Cluster) validateMetricsTLS() error {
	if c.spec.Monitoring.MetricsTLS.SecretName == "" {
		return nil
	}
...
func (c *Cluster) reconcileMetricsService(desired *corev1.Service) error {
	...
	// Merge existing annotations into the desired Service so that annotations
	// set by other operators (e.g. OpenShift serving-cert) are preserved.
	// Desired annotations take precedence over existing ones.
	...
	_, err = k8sutil.UpdateService(c.clusterInfo.Context, c.context.Clientset, c.clusterInfo.Namespace, desired)
	return err
}
