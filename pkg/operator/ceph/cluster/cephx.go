...
func rotateAdminCephxKey(
	...
	// TESTING TESTING TESTING - hack to see what happens on cephcluster update event
	{
		cluster := cephv1.CephCluster{}
		err := clusterdCtx.Client.Get(clusterInfo.Context, clusterInfo.NamespacedName(), &cluster)
		if err != nil {
			panic(err)
		}
		cluster.Spec.Mon.AllowMultiplePerNode = !cluster.Spec.Mon.AllowMultiplePerNode // flip flop
		err = clusterdCtx.Client.Update(clusterInfo.Context, &cluster)
		if err != nil {
			panic(err)
		}
	}
...
func recoverPriorAdminCephxKeyRotation(
	...
	authList, err := client.AuthList(clusterdCtx, clusterInfoCopy)
	if err != nil {
