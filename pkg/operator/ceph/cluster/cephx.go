...
// setDefaultCephxKeyType updates ceph mons to use the given key type by default when creating new
// keys (whether via auth get-or-create or via auth rotate). Ceph config: auth_service_cipher
// Brownfield Ceph clusters do not update this value on their own after mons are upgraded. Rook must
// specify this for Ceph.
// If keyType is given as an empty string, Rook's preferred key type will be set.
// TODO: unit test
func setDefaultCephxKeyType(clusterdCtx *clusterd.Context, clusterInfo *cephclient.ClusterInfo, cephVersion *version.CephVersion, cephxConfig cephv1.ClusterCephxConfig) error {
	if !keyring.Aes256kKeysSupported(*cephVersion) {
	...
	if len(cephxConfig.AllowedCiphers) > 0 {
		...
		logger.Infof("cluster in namespace %q will use the user-specified cephx key cipheer list (auth_allowed_ciphers) %v", clusterInfo.Namespace, allowedCiphers)
	}
	...
	if cephxConfig.Daemon.KeyType != "" {
		logger.Infof("cluster in namespace %q will use the user-specified %q cephx key type (auth_preferred_cipher, auth_service_cipher) by default instead of the Rook default %q key type", clusterInfo.Namespace, cephxConfig.Daemon.KeyType, keyType)
	...
	// also have to update the service cipher type used by ceph internally to the same preference
