...
// createCSIKeyring return four values cephClientID with keyCount as suffix, cephClient key value, bool for should rotate
// and error. It call method `getCsiKeyRotationInfo` to find if any ceph client matching with csi keyBase and max keyGen,
// if no ceph client is found it will return keyGen as 0 and empty list of ceph client if keys are found then based on that
// new key will be generated with new keyGen as suffix and the list of matching ceph client will be deleted later in the method.
// It fetch current the `interpretedCephxStatus` and keyGen from the cephCluster and then call method `ShouldRotateCephxKeys`
// to verify if old key should be rotated. Based on this we update the cephxStatus and generate the new client name and generate new key.
func createCSIKeyring(
	...
	latestClientId = strings.TrimPrefix(latestClientId, "client.")
...
func CreateCSISecrets(context *clusterd.Context, clusterInfo *client.ClusterInfo, clusterNamespaced types.NamespacedName) error {
	...
	logger.Info("getting CephCluster during CSI key creation")
	...
	if err := context.Client.Get(clusterInfo.Context, clusterNamespaced, cephCluster); err != nil {
		...
		return errors.Wrapf(err, "failed to retrieve object store %q to update CSICephxStatus", clusterNamespaced.Namespace)
	}

	// Create CSI RBD Provisioner Ceph key
	rbdProvName, rbdProvKey, rbdProvKeyCount, _, err := createCSIKeyring(context, clusterInfo, cephCluster, k, csiKeyringRBDProvisionerUsername, cephCSIKeyringRBDProvisionerCaps())
	if err != nil {
		return errors.Wrap(err, "failed to create csi rbd provisioner ceph keyring")
	}

	// Create CSI RBD Node Ceph key
	rbdNodeName, rbdNodeKey, rbdNodeKeyCount, _, err := createCSIKeyring(context, clusterInfo, cephCluster, k, csiKeyringRBDNodeUsername, cephCSIKeyringRBDNodeCaps())
	if err != nil {
		return errors.Wrap(err, "failed to create csi rbd node ceph keyring")
	}

	// Create CSI Cephfs provisioner Ceph key
	cFSProvName, cFSProvKey, cFSProvKeyCount, _, err := createCSIKeyring(context, clusterInfo, cephCluster, k, csiKeyringCephFSProvisionerUsername, cephCSIKeyringCephFSProvisionerCaps())
	if err != nil {
		return errors.Wrap(err, "failed to create csi cephfs provisioner ceph keyring")
	}

	// Create CSI Cephfs node Ceph key
	cFSNodeName, cFSNodeKey, cFSNodeKeyCount, shouldRotate, err := createCSIKeyring(context, clusterInfo, cephCluster, k, csiKeyringCephFSNodeUsername, cephCSIKeyringCephFSNodeCaps())
	if err != nil {
		return errors.Wrap(err, "failed to create csi cephfs node ceph keyring")
	}

	csiSecretContent := csiSecretStore{
		CsiRBDProvisionerSecret:    {Name: rbdProvName, Key: rbdProvKey},
		CsiRBDNodeSecret:           {Name: rbdNodeName, Key: rbdNodeKey},
		CsiCephFSProvisionerSecret: {Name: cFSProvName, Key: cFSProvKey},
		CsiCephFSNodeSecret:        {Name: cFSNodeName, Key: cFSNodeKey},
	}
	...
	err = updateCephStatusWithCephxStatus(context, clusterInfo, cephCluster, clusterNamespaced, shouldRotate, currentKeyCount)
	if err != nil {
		return err
	}
...
func updateCephStatusWithCephxStatus(context *clusterd.Context, clusterInfo *client.ClusterInfo, cephCluster *cephv1.CephCluster,
	namespacedName types.NamespacedName, shouldRotate bool, currentKeyCount int,
) error {
	logger.Info("updating cephStatus with CSI cephxStatus")
	cephxStatus := keyring.UpdatedCephxStatus(shouldRotate, cephCluster.Spec.Security.CephX.Daemon, clusterInfo.CephVersion, *cephCluster.Status.Cephx.CSI.CephxStatus)
...
func getCsiKeyRotationInfo(context *clusterd.Context, clusterInfo *client.ClusterInfo, keyBaseName string) (int, []string, error) {
	...
	if err != nil {
		return 0, keyWithBaseName, errors.Wrapf(err, "failed to get sorted CSI key list from %s while reading `maxCount`", sortedClientList)
	}
...
func deduplicate(allKeyWithBaseName []string) []string {
	allKeys := make(map[string]bool)
	uniqueList := []string{}
	for _, v := range allKeyWithBaseName {
