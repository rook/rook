...
func TestGetCsiKeyRotationInfo(t *testing.T) {
	...
	t.Run("keepPriorKeyCountMax` is random value it should not affect the `currentMaxKeyGen`", func(t *testing.T) {
		cSpec := clusterSpec.DeepCopy()
	...
	t.Run("When `authLsList return error`", func(t *testing.T) {
		...
		currentMaxKeyGen, keysWithBaseName, err := getCsiKeyRotationInfo(ctx, clusterInfo, csiKeyringRBDNodeUsername)
		assert.Equal(t, 0, currentMaxKeyGen)
...
func TestGetCSIKeyInfoAndDeleteOldKey(t *testing.T) {
	...
	t.Run("test deletion", func(t *testing.T) {
		...
		executor.MockExecuteCommandWithOutput = func(command string, args ...string) (string, error) {
			...
			if args[0] == "auth" && args[1] == "ls" {
				...
				delKeyArgList = append(delKeyArgList, args[2]) // assuming 2nd arg is key to be deleted
				return "", nil
			}
			panic(fmt.Sprintf("unexpected command %s %v", command, args))
		}
		...
		assert.NoError(t, err)
		wantDeleted := []string{"client.csi-rbd-node", "client.csi-rbd-node.1"}
		assert.Equal(t, wantDeleted, keyDeleted)
		// ensure that `ceph auth delete` was called for the correct keys
		assert.Equal(t, wantDeleted, delKeyArgList)
	})

	t.Run("keep count = to total key count", func(t *testing.T) {
		delKeyArgList := []string{}
		executor.MockExecuteCommandWithOutput = func(command string, args ...string) (string, error) {
			logger.Infof("Command: %s %v", command, args)
			if args[0] == "auth" && args[1] == "ls" {
		...
		ctx.Executor = executor
		keyDeleted, err := deleteOldKeyGen(ctx, clusterInfo, keyWithBaseName, 10)
	...
	t.Run("keep count > to total key count", func(t *testing.T) {
		...
		ctx.Executor = executor
		keyDeleted, err := deleteOldKeyGen(ctx, clusterInfo, keyWithBaseName, 11)
	...
	t.Run("keep count 0", func(t *testing.T) {
		delKeyArgList := []string{}
		executor.MockExecuteCommandWithOutput = func(command string, args ...string) (string, error) {
			logger.Infof("Command: %s %v", command, args)
			if args[0] == "auth" && args[1] == "ls" {
		...
		ctx.Executor = executor
		keyDeleted, err := deleteOldKeyGen(ctx, clusterInfo, keyWithBaseName, 0)

		assert.NoError(t, err)
		wantDeleted := []string{"client.csi-rbd-node", "client.csi-rbd-node.1", "client.csi-rbd-node.2", "client.csi-rbd-node.4", "client.csi-rbd-node.10"}
		assert.Equal(t, wantDeleted, keyDeleted)
		assert.Equal(t, wantDeleted, delKeyArgList)
	})

	t.Run("keep count 1", func(t *testing.T) {
		delKeyArgList := []string{}
		executor.MockExecuteCommandWithOutput = func(command string, args ...string) (string, error) {
			logger.Infof("Command: %s %v", command, args)
			if args[0] == "auth" && args[1] == "ls" {
		...
		ctx.Executor = executor
		keyDeleted, err := deleteOldKeyGen(ctx, clusterInfo, keyWithBaseName, 1)

		assert.NoError(t, err)
		wantDeleted := []string{"client.csi-rbd-node", "client.csi-rbd-node.1", "client.csi-rbd-node.2", "client.csi-rbd-node.4"}
		assert.Equal(t, wantDeleted, keyDeleted)
		assert.Equal(t, wantDeleted, delKeyArgList)
	})

	t.Run("key list empty, keep count 0", func(t *testing.T) {
		delKeyArgList := []string{}
		executor.MockExecuteCommandWithOutput = func(command string, args ...string) (string, error) {
			logger.Infof("Command: %s %v", command, args)
			if args[0] == "auth" && args[1] == "ls" {
		...
		ctx.Executor = executor
		keyDeleted, err := deleteOldKeyGen(ctx, clusterInfo, []string{}, 0)
	...
	t.Run("key list empty, keep count 1", func(t *testing.T) {
		delKeyArgList := []string{}
		executor.MockExecuteCommandWithOutput = func(command string, args ...string) (string, error) {
			logger.Infof("Command: %s %v", command, args)
			if args[0] == "auth" && args[1] == "ls" {
		...
		ctx.Executor = executor
		keyDeleted, err := deleteOldKeyGen(ctx, clusterInfo, []string{}, 1)
	...
	t.Run("key list empty, keep count 3", func(t *testing.T) {
		delKeyArgList := []string{}
		executor.MockExecuteCommandWithOutput = func(command string, args ...string) (string, error) {
			logger.Infof("Command: %s %v", command, args)
			if args[0] == "auth" && args[1] == "ls" {
		...
		ctx.Executor = executor
		keyDeleted, err := deleteOldKeyGen(ctx, clusterInfo, []string{}, 3)
	...
	t.Run("key list with 1 item, keep count 0", func(t *testing.T) {
		delKeyArgList := []string{}
		executor.MockExecuteCommandWithOutput = func(command string, args ...string) (string, error) {
			logger.Infof("Command: %s %v", command, args)
			if args[0] == "auth" && args[1] == "ls" {
		...
		ctx.Executor = executor
		keyDeleted, err := deleteOldKeyGen(ctx, clusterInfo, []string{"client.csi-rbd-node"}, 0)
	...
	t.Run("key list with 1 item, keep count 1", func(t *testing.T) {
		delKeyArgList := []string{}
		executor.MockExecuteCommandWithOutput = func(command string, args ...string) (string, error) {
			logger.Infof("Command: %s %v", command, args)
			if args[0] == "auth" && args[1] == "ls" {
		...
		ctx.Executor = executor
		keyDeleted, err := deleteOldKeyGen(ctx, clusterInfo, []string{"client.csi-rbd-node"}, 1)
	...
	t.Run("key list with 1 item, keep count 3", func(t *testing.T) {
		delKeyArgList := []string{}
		executor.MockExecuteCommandWithOutput = func(command string, args ...string) (string, error) {
			logger.Infof("Command: %s %v", command, args)
			if args[0] == "auth" && args[1] == "ls" {
		...
		ctx.Executor = executor
		keyDeleted, err := deleteOldKeyGen(ctx, clusterInfo, []string{"client.csi-rbd-node"}, 3)
