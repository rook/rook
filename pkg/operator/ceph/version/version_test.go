...
func TestVersionAtLeast(t *testing.T) {
	...
	assert.True(t, (&CephVersion{1, 0, 0}).AtLeast(CephVersion{0, 0, 0}))
	assert.False(t, (&CephVersion{0, 0, 0}).AtLeast(CephVersion{1, 0, 0}))
	assert.True(t, (&CephVersion{1, 1, 0}).AtLeast(CephVersion{1, 0, 0}))
	assert.False(t, (&CephVersion{1, 0, 0}).AtLeast(CephVersion{1, 1, 0}))
	assert.True(t, (&CephVersion{1, 1, 1}).AtLeast(CephVersion{1, 1, 0}))
	assert.False(t, (&CephVersion{1, 1, 0}).AtLeast(CephVersion{1, 1, 1}))
	assert.True(t, (&CephVersion{1, 1, 1}).AtLeast(CephVersion{1, 1, 1}))
}

func TestVersionAtLeastX(t *testing.T) {
	assert.True(t, Nautilus.AtLeastNautilus())
	assert.True(t, Nautilus.AtLeastMimic())
	assert.True(t, Mimic.AtLeastMimic())
	assert.False(t, Luminous.AtLeastMimic())
	assert.False(t, Luminous.AtLeastNautilus())
	assert.False(t, Mimic.AtLeastNautilus())
}
