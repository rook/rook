...
func TestFilterDestroyedOSDIdsForNodeDeepHierarchy(t *testing.T) {
	...
	t.Run("recurses through an intermediate bucket below the host", func(t *testing.T) {
		// node-b2's children are a chassis bucket, not osds directly; osd.6 is one level deeper.
		// This is load-bearing for collectOSDsUnderBucket's recursion: a non-recursive walk that
		// only inspects the host's direct children would miss osd.6 and return empty here. (This
		// nested-below-host chassis shape is non-standard CRUSH but guards the recursive walk.)
		ids, err := filterDestroyedOSDIdsForNode(tree, destroyed, "root=default datacenter=dc2 rack=rack-b1 host=node-b2")
		assert.NoError(t, err)
		assert.Equal(t, []int{6}, ids)
	})
}
