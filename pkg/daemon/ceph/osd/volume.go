...
func GetCephVolumeRawOSDs(context *clusterd.Context, clusterInfo *client.ClusterInfo, cephfsid, block, metadataBlock, walBlock string, lvBackedPV, skipDeviceClass bool, devices []DesiredDevice) ([]oposd.OSDInfo, error) {
	...
	var err error
	if block == "" {
		// A no-argument "ceph-volume raw list" enumerates every block device in one batched
		// ceph-bluestore-tool call; on Ceph v20.2.x a single unreadable device (canonically a
		// sub-4KiB MBR extended-partition node) crashes that call and blanks the entire listing
		// to "{}" with a zero exit code (https://tracker.ceph.com/issues/76354, rook #17992).
		// List each device individually instead so one bad device cannot hide the OSDs on the rest.
...
// and merges the results, keyed by OSD UUID as the raw-list output is. Listing per device is immune
// to the failure that blanks the no-argument "ceph-volume raw list": on Ceph v20.2.x a single
// device that crashes ceph-bluestore-tool's batched show-label makes the whole no-argument listing
// return "{}" with a zero exit code (https://tracker.ceph.com/issues/76354, rook #17992). A device
// that fails to list is logged and skipped so one bad device cannot hide the OSDs on the others.
