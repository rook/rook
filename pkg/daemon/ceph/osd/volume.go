...
// matchPerDeviceClass returns the CR-declared deviceClass for the OSD, or "" if no entry matches.
func matchPerDeviceClass(perDevice []DesiredDevice, blockPath, kernelName, devLinks string) string {
...
func GetCephVolumeRawOSDs(context *clusterd.Context, clusterInfo *client.ClusterInfo, cephfsid, block, metadataBlock, walBlock string, lvBackedPV, skipDeviceClass bool, perDevice []DesiredDevice) ([]oposd.OSDInfo, error) {
	// lv can be a block device if raw mode is used
	cvMode := "raw"
	...
	for _, osdInfo := range cephVolumeResult {
		...
		if !skipDeviceClass {
			...
			if dc := matchPerDeviceClass(perDevice, blockPath, diskInfo.KernelName, diskInfo.DevLinks); dc != "" {
				osd.DeviceClass = dc
				logger.Infof("setting device class %q from CR per-device spec for device %q", osd.DeviceClass, diskInfo.Name)
			} else {
				if len(perDevice) > 0 {
					logger.Debugf("no CR per-device spec matched blockPath=%q kernel=%q devLinks=%q; falling back to disk-derived class", blockPath, diskInfo.KernelName, diskInfo.DevLinks)
				}
