...
func TestInitKeyProtect(t *testing.T) {
	...
	t.Run("IBM_SERVICE_API_KEY not set", func(t *testing.T) {
		_, err := InitKeyProtect(config)
		assert.Error(t, err)
		assert.ErrorIs(t, err, ibm.ErrIbmServiceApiKeyNotSet)
		config[ibm.IbmServiceApiKey] = "foo"
	})

	t.Run("IBM_INSTANCE_ID not set", func(t *testing.T) {
		_, err := InitKeyProtect(config)
		assert.Error(t, err)
		assert.ErrorIs(t, err, ibm.ErrIbmInstanceIdKeyNotSet)
		config[ibm.IbmInstanceIdKey] = "foo"
	})
	...
	t.Run("different base URL", func(t *testing.T) {
		config[ibm.IbmBaseUrlKey] = "https://us-west.kms.cloud.ibm.com"
		c, err := InitKeyProtect(config)
		assert.NoError(t, err)
