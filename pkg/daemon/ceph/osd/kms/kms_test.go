...
func TestValidateConnectionDetails(t *testing.T) {
	...
	t.Run("ibm kp - token present but no key for service key", func(t *testing.T) {
		...
		err = ValidateConnectionDetails(ctx, context, ibmSecuritySpec, ns)
		assert.Error(t, err, "")
		assert.EqualError(t, err, "failed to read k8s kms secret \"IBM_SERVICE_API_KEY\" key \"ibm-token\" (not found or empty)")
	})

	t.Run("ibm kp - token ok but no instance id", func(t *testing.T) {
		ibmSecret.Data["IBM_SERVICE_API_KEY"] = []byte("foo")
		_, err := context.Clientset.CoreV1().Secrets(ns).Update(ctx, ibmSecret, metav1.UpdateOptions{})
		assert.NoError(t, err)
		err = ValidateConnectionDetails(ctx, context, ibmSecuritySpec, ns)
		assert.Error(t, err, "")
		assert.EqualError(t, err, "failed to validate kms config \"IBM_INSTANCE_ID\". cannot be empty")
		ibmSecuritySpec.KeyManagementService.ConnectionDetails["IBM_INSTANCE_ID"] = "foo"
	})

	t.Run("ibm kp - success", func(t *testing.T) {
		err := ValidateConnectionDetails(ctx, context, ibmSecuritySpec, ns)
		assert.NoError(t, err, "")
		// IBM_SERVICE_API_KEY must be appended to the details so that the client can be built with
		// all the details
		assert.Equal(t, ibmSecuritySpec.KeyManagementService.ConnectionDetails["IBM_SERVICE_API_KEY"], "foo")
	})
}
