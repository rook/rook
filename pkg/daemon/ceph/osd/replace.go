...
func (a *OsdAgent) preProvisionReplacedOSDs(context *clusterd.Context, available *DeviceOsdMapping, crushLocation string) error {
	...
	for _, osdID := range destroyedOSDIds {
		...
		if err := a.provisionReplacedOSD(context, osdID, available.Entries[deviceName]); err != nil {
			return errors.Wrapf(err, "failed to provision replacement for destroyed osd.%d on device %q", osdID, deviceName)
		}
		// Drop the consumed device so the normal provisioning path does not re-claim it as a new OSD.
...
//
// TODO: should we harden the pairing by matching a specific physical device to a specific id (e.g. via
// udev/by-path) rather than pairing any blank device with any slot?
...
func filterDestroyedOSDIdsForNode(tree client.OsdTree, destroyedOSDIds []int, crushLocation string) ([]int, error) {
	...
	// Collect every osd-type descendant under this node's host bucket, recursing through any
	// intermediate buckets (e.g. chassis/rack) so the hierarchy below the host is handled.
	hostOSDs := map[int]struct{}{}
	for _, node := range tree.Nodes {
		if node.Type == "host" && node.Name == hostName {
			collectOSDsUnderBucket(tree, node.Children, hostOSDs)
			break
		}
	}
	...
	for _, id := range destroyedOSDIds {
		if _, ok := hostOSDs[id]; ok {
			nodeDestroyed = append(nodeDestroyed, id)
		}
	}

	return nodeDestroyed, nil
}

// collectOSDsUnderBucket walks the CRUSH subtree rooted at the given child ids and records the ids
// of all osd-type nodes into found, recursing through intermediate buckets.
func collectOSDsUnderBucket(tree client.OsdTree, childIDs []int, found map[int]struct{}) {
	byID := make(map[int]int, len(tree.Nodes))
...
func (a *OsdAgent) recoverDBLVForOSDFromHost(context *clusterd.Context, osdID int) (string, error) {
	result, err := callCephVolume(context, "lvm", "list", strconv.Itoa(osdID), "--format", "json")
	if err != nil {
		return "", errors.Wrapf(err, "failed to list lvm volumes for osd.%d", osdID)
	}

	var listResult map[string][]cvLVMListEntry
	if err := json.Unmarshal([]byte(result), &listResult); err != nil {
		return "", errors.Wrapf(err, "failed to unmarshal ceph-volume lvm list result for osd.%d. %s", osdID, result)
	}

	for _, entry := range listResult[strconv.Itoa(osdID)] {
		if entry.Type != "db" {
			continue
		}
