...
func getDeviceEmpty(device *LocalDisk) bool {
	return device.Parent == "" && (device.Type == sys.DiskType || device.Type == sys.SSDType || device.Type == sys.CryptType || device.Type == sys.LvType) && device.FileSystem == ""
}
...
func DiscoverDevices(executor exec.Executor) ([]*LocalDisk, error) {
	...
	for _, d := range devices {
		...
		if !ok || (diskType != sys.SSDType && diskType != sys.CryptType && diskType != sys.LvType && diskType != sys.DiskType && diskType != sys.PartType) {
