...
func addOSDConfigFlags(command *cobra.Command) {
	...
	command.Flags().StringVar(&cfg.storeConfig.DmcryptFormatOptions, "dmcrypt-format-opts", "", "options for dm-crypt LUKS format command")
	command.Flags().StringVar(&cfg.storeConfig.DmcryptOpenOptions, "dmcrypt-open-opts", "", "options for dm-crypt LUKS open command")
