...
var filesystemMountCmd = &cobra.Command{
	Use:   "mount",
	Short: "Mounts a shared filesystem from the cluster to the given local mount point",
}

func init() {
	filesystemMountCmd.Flags().StringVarP(&mountFilesystemName, "name", "n", "", "Name of filesystem to mount (required)")
	filesystemMountCmd.Flags().StringVarP(&mountFilesystemPath, "path", "p", "", "Path to mount shared filesystem (required)")

	filesystemMountCmd.MarkFlagRequired("name")
	filesystemMountCmd.MarkFlagRequired("path")
	filesystemMountCmd.RunE = mountFilesystemEntry
}
