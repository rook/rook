...
Today, the `rook-ceph-admin-keyring` secret ("the secret") contains the `client.admin` keyring
...
While `client.admin-rotator` exists, it must be stored in the secret also. The secret's new data
format will look as follows:

- `data`
    - `keyring`: contains the primary `client.admin` keyring
    - `rotatorKeyring`: if present, contains the temporary `client.admin-rotator` keyring
...
2.  store/update the `client.admin-rotator` key into the secret `data.rotatorKeyring`
3.  read back `client.admin-rotator` keyring from the secret, and verify it
4.  **ROTATE**: as `client.admin-rotator`: run `ceph auth ls` to ensure it has permissions
5.  as `client.admin-rotator`: run `ceph auth rotate client.admin`
6.  as `client.admin` run `ceph auth ls` to ensure it has permissions
7.  update on-disk ceph config file with new `client.admin` keyring
8.  update the secret `data.keyring` with new `client.admin` keyring
9.  read back `client.admin` keyring from the secret, and verify it
10. as `client.admin`: run `ceph auth rm client.admin-rotator`
11. remove `data.rotatorKeyring` from the secret
12. update `CephCluster.status.cephx.admin` with updated CephX status
...
1. read the secret (same as today)
    1. if secret `data.rotatorKeyring` is not present, no recovery needed
        1. load `client.admin` from the secret `data.keyring` and continue reconcile (same as today)
    2. if secret `data.rotatorKeyring` is present, prior rotation failed somewhere -- now recover
