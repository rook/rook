...
3.  read back the `client.admin-rotator` keyring from its secret, and verify it
...
7.  update the on-disk ceph config file with new `client.admin` keyring
8.  update the `rook-ceph-mon` secret with new `client.admin` keyring
9.  read back the `client.admin` keyring from its secret, and verify it
10. as `client.admin`: run `ceph auth rm client.admin-rotator`
11. **CLEANUP** delete the `rook-ceph-admin-keyring` secret
