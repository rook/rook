...
### RGW Deployments
- If `TrafficDistribution:PreferClose`is enabled then one RGW deployment will be created for topology domain.
- For example, if `failureDomain` is set to `zone`, then use zone specific deployments to create a separate RGW deployment for each zone. - By default, each deployment should have one replica.
- The deployment in one locality can be scaled independent of the other localities.
