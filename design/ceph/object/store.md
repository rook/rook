...
Rook will apply `rgwConfig` values to the mon config store after its own values are set. Similarly,
...
Configs set via `rgwConfig` will be equivalent to a Ceph config section like the following ini. Rook
will need to do a small amount of string parsing to ensure CLI flags use hyphens (e.g.,
`debug-rgw`), but config values can use spaces, hyphens, or underscores. Rook will choose
underscores (e.g., `debug_rgw`) to keep consistency. Rook's API will allow users to use dashes or
underscores to separate words for ease of use, but not spaces which could affect YAML/JSON parsing.
