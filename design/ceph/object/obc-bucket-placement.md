...
rook-side control over who may set the keys — the value pattern below is
the only control over what they may say — and it is process-wide and keyed
by name, so enabling a key enables it for every OBC in every namespace.
...
placement; the pre-existing `PoolPlacementSpec.name` and
`PlacementStorageClassSpec.name` patterns both permit `/`, so a
slash-bearing target or class cannot be requested here. `:` is excluded for
the same reason on the other side of the wire: it separates the zonegroup
from the placement in the location constraint. Whitespace is excluded
...
today treats as an idempotent success. Under this design that answer, when a
placement or class was requested, is a reconcile error naming them; a
same-owner, same-config re-create is answered with 200 on every supported
release, so no idempotent path is lost. (`BucketAlreadyOwnedByYou`, the
other 409, stays an idempotent success; RGW does not emit it.) This is a
backstop rather than the main path: the comparison below runs first, so
`CreateBucket` is reached only for a bucket the exists check did not see.
...
- If `bucketPlacement` is set and differs from the reported placement name,
    ...
    the requested and the actual values, before the claim's RGW user is
    created and therefore before any relink, quota, policy or lifecycle
    write, so a claim the bucket can never satisfy leaves nothing behind.
...
RGW's refusal of the requested placement or class at creation (the codes it
refuses them with, the 409 included, each carrying RGW's message; a failure
to reach RGW is not a refusal), and the existing-bucket
