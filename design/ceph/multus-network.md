...
If an error occurs during the migration job, the operator will run a teardown job on the the node that the error occurred, log the error, and continue. The teardown job runs on the host network namespace of the node. It searches for the migrated interface in the host network namespace and removes it. If there is no such interface present, the job is considered complete. The multus holder pod is then deleted from the node, causing a new pod with a fresh multus IP configuration to be brought up. This cleanup puts the node in the best state to try to rerun the migration job, which the controller will do in a future reconcile loop. Logging and continuing in the current loop allows the nodes that migrated successfully to continue with the CSI setup.
