...
git_longhaul_test_directory=github.com/rook/rook/e2e/tests/block/k8s/longhaul
...
rook_infra::run_longhul() {
    local test_name_regex=$1
    local total_runs=$2
    local test_runs=$3
    local parallel_runs=$4
    local test_intervel=$5
    for (( totalruns=1; totalruns<=${total_runs}; totalruns++ ))
	do

   		docker exec -t ${id} /bin/bash -c \
        	"go test -run ${test_name_regex} ${git_longhaul_test_directory} --skip_install_rook=true --load_parallel_runs=${parallel_runs} -count=${test_runs} -v 2>&1 | tee ${results_dir}/${longhaul_result_name}${totalruns}.log"
         docker exec ${id} /bin/bash -c \
        	"go get -u -f github.com/jstemmer/go-junit-report && cat ${results_dir}/${longhaul_result_name}${totalruns}.log | go-junit-report > ${results_dir}/LongHauljunit${totalruns}.xml"

    	rc=$?; if [[ $rc != 0 ]]; then set -e; fi
    	sleep ${test_intervel}m
	done
}
...
{ #try
    ...
    rook_infra::run_test TestBlockStorageSmokeSuite ${tag_name} ${rook_platform} ${k8s_version}
...
echo "Starting long haul test"
a=$SECONDS
rook_infra::run_longhul Test ${total_runs} ${total_per_runs} ${parallel_runs}  ${test_intervel}
elapsedseconds=$(( SECONDS - a ))

echo " Long haul Test completed - Ran for $elapsedseconds seconds"
