...
func (r *rookTestInfraManager) ValidateAndPrepareEnvironment() error	{
	...
	dockerClient.ExecuteCmd([]string{km_dockId,"/bin/bash","-c","'apt-get -y update && apt-get install -qqy ceph-common'"})

	kn1_dockId,_,_ := dockerClient.ExecuteCmd([]string {"ps", "--filter", "name=kube-node-1", "--format",  "\"{{.ID}}\""})
	dockerClient.ExecuteCmd([]string{kn1_dockId,"/bin/bash","-c","'apt-get -y update && apt-get install -qqy ceph-common'"})
