...
type Irook_filesystem interface {
	...
	FS_Write(name string, mountpath string, data string, filename string, namespace string) (string, error)
	FS_Read(name string, mountpath string, filename string, namespace string) (string, error)
