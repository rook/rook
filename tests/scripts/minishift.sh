...
function init_flexvolume() {
    cat <<EOF | ssh -i `minishift ssh-key` docker@`minishift ip` -o IdentitiesOnly=yes -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -o LogLevel=quiet 'cat - > ~/rook'
#!/bin/bash
echo -ne '{"status": "Success", "capabilities": {"attach": false}}' >&1
exit 0
EOF
    minishift ssh "chmod +x ~/rook"
    minishift ssh "sudo chown root:root ~/rook"
    minishift ssh "sudo mkdir -p /usr/libexec/kubernetes/kubelet-plugins/volume/exec/rook.io~rook"
    minishift ssh "sudo mv ~/rook /usr/libexec/kubernetes/kubelet-plugins/volume/exec/rook.io~rook"
    minishift start --memory=${MEMORY} --vm-driver=virtualbox
}
