...
func (h *CephInstaller) createMultisiteObjectStoreConfiguration(values map[string]any, name, scName string) error {
	poolNames := rgwSharedPools(name)
	rgwPools := make([]map[string]any, 0, len(poolNames))
	for poolName, radosName := range poolNames {
		...
		rgwPools = append(rgwPools, map[string]any{
			"name": poolName,
			"spec": spec,
			"storageClass": map[string]any{
				"enabled": false,
	...
	existingPools, _ := values["cephBlockPools"].([]map[string]any)
	values["cephBlockPools"] = append(existingPools, rgwPools...)

	values["cephObjectRealms"] = []map[string]any{
		{
			"name": name,
		},
	}
	values["cephObjectZoneGroups"] = []map[string]any{
		{
			"name": name,
			"spec": map[string]any{
				"realm": name,
	...
	values["cephObjectZones"] = []map[string]any{
		{
			"name": name,
			"spec": map[string]any{
				"zoneGroup": name,
				"sharedPools": map[string]any{
					"poolPlacements": []map[string]any{
						{
							"name":             "default",
							"default":          true,
