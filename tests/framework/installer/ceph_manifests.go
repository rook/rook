...
func NewCephManifests(version string) CephManifests {
	...
	case Version1_5:
		return &CephManifestsV1_5{imageTag: Version1_5}
	}
...
func (m *CephManifestsMaster) GetRookCRDs(v1ExtensionsSupported bool) string {
	if v1ExtensionsSupported {
		return readManifest("crds.yaml")
	}
	return readManifest("pre-k8s-1.16/crds.yaml")
}

func getOpenshiftSCC(namespace string) string {
	manifest := readManifest("scc.yaml")
	return strings.ReplaceAll(manifest, "system:serviceaccount:rook-ceph:", fmt.Sprintf("system:serviceaccount:%s:", namespace))
}
