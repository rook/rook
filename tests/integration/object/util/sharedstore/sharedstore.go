...
func Create(t *testing.T, k8sh *utils.K8sHelper, installer *installer.CephInstaller) *Sharedstore {
	...
	pools := []*cephv1.CephBlockPool{
		...
		{
			ObjectMeta: metav1.ObjectMeta{
				Name:      storeName + ".rgw.buckets.index",
				Namespace: ns,
			},
			Spec: cephv1.NamedBlockPoolSpec{
				PoolSpec: cephv1.PoolSpec{
		...
		{
			ObjectMeta: metav1.ObjectMeta{
				Name:      storeName + ".rgw.buckets.data",
				Namespace: ns,
			},
			Spec: cephv1.NamedBlockPoolSpec{
				PoolSpec: cephv1.PoolSpec{
		...
		{
			ObjectMeta: metav1.ObjectMeta{
				Name:      storeName + ".rgw.buckets.data.foo",
				Namespace: ns,
			},
			Spec: cephv1.NamedBlockPoolSpec{
				PoolSpec: cephv1.PoolSpec{
	...
	s.destroy = func() {
		t.Run(fmt.Sprintf("destroy CephObjectStore %s", objectStore.Name), func(t *testing.T) {
			...
			wait4.AssertDelete(ctx, t, ceph.CephObjectStores(ns), objectStore.Name, 2*time.Minute)
