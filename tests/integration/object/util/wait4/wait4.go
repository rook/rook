...
func waitForAbsent[T, L runtime.Object](ctx context.Context, client NamespacedWatcher[T, L], name string, timeout time.Duration) error {
	var objType T
...
func waitForCondition[T, L runtime.Object](ctx context.Context, client NamespacedWatcher[T, L], name string, cond func(T) bool, timeout time.Duration) (T, error) {
	var objType T
	var match T
	lw := nameScopedListWatch(ctx, client, name)
...
func deleteAndWait[T, L runtime.Object](ctx context.Context, client NamespacedDeleter[T, L], name string, timeout time.Duration) error {
	var objType T
	if err := client.Delete(ctx, name, metav1.DeleteOptions{}); err != nil {
		return fmt.Errorf("failed to delete %T %q: %w", objType, name, err)
	}
	return waitForAbsent[T, L](ctx, client, name, timeout)
}
...
func AssertDelete[T, L runtime.Object](
	...
	t.Helper()
	if err := deleteAndWait(ctx, client, name, timeout); err != nil {
...
func RequireDelete[T, L runtime.Object](
	...
	t.Helper()
	require.NoError(t, deleteAndWait(ctx, client, name, timeout), msgAndArgs...)
}
...
func AssertAbsent[T, L runtime.Object](
	...
	t.Helper()
	if err := waitForAbsent(ctx, client, name, timeout); err != nil {
		return assert.Fail(t, err.Error(), msgAndArgs...)
	}
	return true
}
...
func RequireAbsent[T, L runtime.Object](
	...
	t.Helper()
	require.NoError(t, waitForAbsent(ctx, client, name, timeout), msgAndArgs...)
}
...
func createAndWaitReady[T, L runtime.Object](
	...
	return waitForCondition[T, L](ctx, client, accessor.GetName(), ready, timeout)
}
...
func AssertCreate[T, L runtime.Object](
	...
	t.Helper()
	live, err := createAndWaitReady(ctx, client, obj, ready, timeout)
	if err != nil {
		var zero T
...
func AssertCondition[T, L runtime.Object](
	...
	t.Helper()
	match, err := waitForCondition(ctx, client, name, cond, timeout)
	if err != nil {
		var zero T
		assert.Fail(t, err.Error(), msgAndArgs...)
		return zero, false
