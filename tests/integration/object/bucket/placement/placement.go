...
// The shared store's placement fixtures this package provisions against:
// "default" is its default placement target and carries the non-default
// "FOO" storage class; "bar" is its second, non-default target.
const (
...
func TestObjectBucketClaimPlacement(t *testing.T, k8sh *utils.K8sHelper, store *sharedstore.Sharedstore) {
	var (
		...
		obcBar            = claim(defaultName+"-bar", storageClass.Name, map[string]string{"bucketPlacement": placementBar})
		obcFoo            = claim(defaultName+"-foo", storageClass.Name, map[string]string{"bucketStorageClass": storageClassFoo})
		obcBoth           = claim(defaultName+"-both", storageClass.Name, map[string]string{"bucketPlacement": placementDefault, "bucketStorageClass": storageClassFoo})
		...
		// brownfield: a Grant on obcBar's bucket, which is on the bar placement
		grantClass = grantStorageClass(defaultName+"-grant", objectStore, obcBar.Spec.BucketName)
		obcGrant   = grantClaim(defaultName+"-grant", grantClass.Name, map[string]string{"bucketPlacement": placementDefault})

		fooRule = placementDefault + "/" + storageClassFoo
	...
	t.Run("OBC bucketPlacement", func(t *testing.T) {
		...
		obc.RequireBound(ctx, t, k8sh, obcBar)
		checkPlacementRule(t, adminClient, obcBar.Spec.BucketName, placementBar)
		...
		t.Run(fmt.Sprintf("update obc %q to bucketPlacement %q", obcBoth.Name, placementBar), func(t *testing.T) {
			obc.Update(ctx, t, k8sh, ns.Name, obcBoth.Name, func(live *bktv1alpha1.ObjectBucketClaim) {
				live.Spec.AdditionalConfig["bucketPlacement"] = placementBar
		...
		checkWarningEvent(t, k8sh, obcBoth, reasonPlacementMismatch, placementBar, placementDefault)
		...
		t.Run(fmt.Sprintf("revert obc %q to bucketPlacement %q", obcBoth.Name, placementDefault), func(t *testing.T) {
			obc.Update(ctx, t, k8sh, ns.Name, obcBoth.Name, func(live *bktv1alpha1.ObjectBucketClaim) {
				live.Spec.AdditionalConfig["bucketPlacement"] = placementDefault
			})
		})
		...
		t.Run(fmt.Sprintf("create obc %q granting bucket %q with bucketPlacement %q", obcGrant.Name, obcBar.Spec.BucketName, obcGrant.Spec.AdditionalConfig["bucketPlacement"]), func(t *testing.T) {
			...
			bucket, err := adminClient.GetBucketInfo(ctx, admin.Bucket{Bucket: obcBar.Spec.BucketName})
			require.NoError(t, err)
		...
		checkWarningEvent(t, k8sh, obcGrant, reasonPlacementMismatch, placementDefault, placementBar)
		checkPhase(t, k8sh, obcGrant, bktv1alpha1.ObjectBucketClaimStatusPhasePending)

		t.Run(fmt.Sprintf("update obc %q to bucketPlacement %q", obcGrant.Name, placementBar), func(t *testing.T) {
		...
		t.Run(fmt.Sprintf("obc %q binds once the placement matches", obcGrant.Name), func(t *testing.T) {
			...
			assert.Equal(t, placementBar, bucket.PlacementRule)
			assert.Equal(t, obcGrant.Spec.AdditionalConfig["bucketOwner"], bucket.Owner)
		})

		// the grant claim goes first: deleting obcBar removes the bucket they
		// share
		for _, name := range []string{obcGrant.Name, obcBar.Name, obcFoo.Name, obcBoth.Name} {
