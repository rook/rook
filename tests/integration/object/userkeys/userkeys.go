	...
	t.Run("ObjectStoreUser keys", func(t *testing.T) {
		...
		t.Run(fmt.Sprintf("expected keys set on user %q", osu1.Name), func(t *testing.T) {
			...
			for _, secret := range secretKeys {
				k, err := findUserKeySpec(liveUser.Keys, string(secret.Data["AWS_ACCESS_KEY_ID"]))
				require.NoError(t, err)
				assert.Equal(t, string(secret.Data["AWS_ACCESS_KEY_ID"]), k.AccessKey)
				assert.Equal(t, string(secret.Data["AWS_SECRET_ACCESS_KEY"]), k.SecretKey)
			}
		})
		...
		t.Run(fmt.Sprintf("keys set on user %q changed", osu1.Name), func(t *testing.T) {
			...
			// the 5 explicit keypairs are set
			secretKeys := []*corev1.Secret{secret1, secret2, secret3, secret4, secret5}
		...
		t.Run(fmt.Sprintf("keys set on user %q changed", osu1.Name), func(t *testing.T) {
			...
			secretKeys := []*corev1.Secret{liveSecret, secret2, secret3, secret4, secret5}

			// wait for the reconcile to run for the 1 secret
			inSync := utils.Retry(40, time.Second, "CephObjectStoreUser keys changed", func() bool {
				var err error
				liveUser, err = adminClient.GetUser(ctx, admin.User{ID: osu1.Name})
				if err != nil {
				...
				_, err = findUserKeySpec(liveUser.Keys, string(liveSecret.Data["AWS_ACCESS_KEY_ID"]))
				return err == nil
			})
			require.True(t, inSync)
			assert.Equal(t, len(secretKeys), len(liveUser.Keys))
