...
func TestCephObjectStoreDependents(t *testing.T, k8sh *utils.K8sHelper, store *sharedstore.Sharedstore) {
	var (
		...
		// rather than the shared fixture; it stays plain http in both suite
		// passes — the TLS rgw surfaces are covered by the shared-store packages
	...
	t.Run("CephObjectStore dependents", func(t *testing.T) {
		...
		// The scenario deletes the store and user itself; if it aborts partway
		// (e.g. the obc never binds) their fixed names would collide with the
		// suite's other TLS pass, so sweep any leftovers.
		t.Cleanup(func() {
			ctx := context.Background()
			_ = osuClient.Delete(ctx, user1.Name, metav1.DeleteOptions{})
			_ = cosClient.Delete(ctx, privateStore.Name, metav1.DeleteOptions{})
			_ = wait.PollUntilContextTimeout(ctx, 5*time.Second, 3*time.Minute, true, func(ctx context.Context) (bool, error) {
				_, err := cosClient.Get(ctx, privateStore.Name, metav1.GetOptions{})
