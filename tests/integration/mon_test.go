...
func (suite *SmokeSuite) TestMonFailover() {
	...
	monReplicas, err := suite.k8sh.Clientset.ReplicaSets(defaultRookNamespace).List(opts)
	...
	err = suite.k8sh.Clientset.ReplicaSets(defaultRookNamespace).Delete(monToKill, delOptions)
	require.Nil(suite.T(), err)
