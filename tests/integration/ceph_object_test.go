...
func (s *ObjectSuite) TestWithTLS() {
	...
	runObjectE2ETest(s.k8sh, s.installer, &s.Suite, s.settings.Namespace, true)
}
...
func (s *ObjectSuite) TestWithoutTLS() {
	...
	runObjectE2ETest(s.k8sh, s.installer, &s.Suite, s.settings.Namespace, false)
}

func runObjectE2ETest(k8sh *utils.K8sHelper, installer *installer.CephInstaller, s *suite.Suite, namespace string, tlsEnable bool) {
	sharedObjectStore := sharedstore.Create(s.T(), k8sh, installer, tlsEnable, namespace, "sharedstore", 1,
