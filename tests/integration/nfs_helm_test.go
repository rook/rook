...
// *******************************************************
// *** Major scenarios tested by the NfsHelmSuite ***
// Setup
...
// *******************************************************
func TestNfsHelmSuite(t *testing.T) {
	if installer.SkipTestSuite(installer.NFSTestSuite) {
		t.Skip()
	}

	s := new(NfsHelmSuite)
	defer func(s *NfsHelmSuite) {
		HandlePanics(recover(), s, s.T)
	}(s)
	suite.Run(t, s)
}

type NfsHelmSuite struct {
	suite.Suite
...
func (suite *NfsHelmSuite) SetupSuite() {
	suite.Setup()
}

func (suite *NfsHelmSuite) TearDownSuite() {
	suite.Teardown()
}

func (suite *NfsHelmSuite) Setup() {
...
func (suite *NfsHelmSuite) Teardown() {
...
func (suite *NfsHelmSuite) TestNfsServerInstallation() {
...
func (suite *NfsHelmSuite) checkReadData(podList []string) bool {
	inc := 0
	var result string
	var err error
