...
3. Kubernetes with `kubectl` configured / OpenShift with `oc` configured (in case of testing on OpenShift)
4. Rook
...
You can also set some ENV variables before the execution to change the behavior of the framework.
(e.g. `export ROOK_TEST_OPENSHIFT=true`)

| Variable            | Description                                     | Possible values  |
| --------------------| ------------------------------------------------| ---------------- |
| ROOK_TEST_OPENSHIFT | set this value to true for testing on OpenShift | true             |
...
- Export environment variable: `export ROOK_TEST_OPENSHIFT=true`
- Make sure `oc` executable file is in your PATH
- `go test -timeout 1800s -v -run TestCeph github.com/rook/rook/tests/integration`
