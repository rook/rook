...
rook-ceph-nvmeof-nvmeof-a-lb   LoadBalancer   172.30.149.173   fa38eb00-us-east.lb.appdomain.cloud   4420:30601/TCP,5500:31856/TCP,8009:30144/TCP
```
...
### Mount the volume read-only inside the pod

> **Warning:** Never mount ext4 read-write from two hosts simultaneously.
> This will corrupt the filesystem. Use read-only mode when verifying.

```bash
# First sync data on the external client
sync /mnt/nvmeof/

# Then mount read-only in the pod
...
> **Note:** `echo 3 > /proc/sys/vm/drop_caches` flushes the kernel page cache to
> ensure the pod reads fresh data from the Ceph backend, not stale cached blocks.

### Real-world test: verifying data from inside the pod

After writing `new.txt` on the external PC, exec into the debug pod to confirm
the data is visible:
...
```
 External PC                              Kubernetes Cluster Pod
 ──────────                               ───────────────
...
### Data not visible after writing from another host

ext4 does not support shared access. When one host writes data, other hosts with the
same filesystem mounted will not see the changes due to kernel block cache. To refresh:

1. **Sync on the writer:** `sync /mnt/nvmeof/`
2. **On the reader:** unmount, drop caches, and remount:

```bash
umount /mnt/nvmeof
echo 3 > /proc/sys/vm/drop_caches
mount -o ro /dev/nvmeXnY /mnt/nvmeof
```
