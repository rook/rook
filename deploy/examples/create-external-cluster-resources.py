...
class RadosJSON:
    ...
    def gen_arg_parser(cls, args_to_parse=None):
        ...
        output_group.add_argument(
            ...
            + "For usage, we need to set `--cephx-key-rotation rotate` so key rotation can happen"
            + "In case of reverting the keys to the one older key, we need to set `--cephx-key-rotation revert`",
        )
    ...
    def get_cephx_latest_key_genration(self):
        generation = 0
    ...
    def gen_json_out(self, generation):
        ...
        if (
            ...
            json_out.append(
                {
                    "name": self.out_map["CSI_RBD_NODE_SECRET_NAME"],
                    "kind": "Secret",
                    "data": {
        ...
        if (
            ...
            json_out.append(
                {
                    "name": self.out_map["CSI_RBD_PROVISIONER_SECRET_NAME"],
                    "kind": "Secret",
                    "data": {
                        "userID": self.getUserId(
        ...
        if (
            ...
            json_out.append(
                {
                    "name": self.out_map["CSI_CEPHFS_PROVISIONER_SECRET_NAME"],
                    "kind": "Secret",
                    "data": {
                        "adminID": self.getUserId(
                            self.out_map["CSI_CEPHFS_PROVISIONER_SECRET_NAME"],
                            generation,
                        ),
        ...
        if (
            ...
            json_out.append(
                {
                    "name": self.out_map["CSI_CEPHFS_NODE_SECRET_NAME"],
                    "kind": "Secret",
                    "data": {
                        "adminID": self.getUserId(
            ...
            json_out.append(
                {
                    ...
                    "data": {
                        "pool": self.out_map["RBD_POOL_NAME"],
                        "csi.storage.k8s.io/provisioner-secret-name": self.out_map[
                            "CSI_RBD_PROVISIONER_SECRET_NAME"
                        ],
                        "csi.storage.k8s.io/controller-expand-secret-name": self.out_map[
                            "CSI_RBD_PROVISIONER_SECRET_NAME"
                        ],
                        "csi.storage.k8s.io/node-stage-secret-name": self.out_map[
    ...
    def getUserId(self, user_name, generation):
        """
    ...
    def main(self):
        ...
        generation = 0
        current_generation = self.get_cephx_latest_key_genration()
