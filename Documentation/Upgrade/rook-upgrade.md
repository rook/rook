...
* S3 users provisioned via CephObjectStoreUser resources no longer allow multiple credentials to
    exist on underlying S3 users. Rook will purge all but one of the credentials. This could be a
    user observable regression for administrators who manually edited/rotated S3 user credentials
    for CephObjectStoreUsers. For affected users, Rook now provides first-class
