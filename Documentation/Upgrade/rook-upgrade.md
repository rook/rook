...
example, when Rook v1.11.6 is released, the process of updating from v1.11.0 is as simple as running
the following:

```console
git clone --single-branch --depth=1 --branch v1.11.6 https://github.com/rook/rook.git
...
kubectl -n rook-ceph set image deploy/rook-ceph-operator rook-ceph-operator=rook/ceph:v1.11.6
```
...
the version `v1.11.6`. This upgrade should work from any official patch release of Rook v1.10 to any
...
```console
kubectl -n $ROOK_OPERATOR_NAMESPACE set image deploy/rook-ceph-operator rook-ceph-operator=rook/ceph:v1.11.6
```
...
availability and `rook-version=v1.11.6`, the Ceph cluster's core components are fully updated.
...
rook-ceph-osd-0         req/upd/avl: 1//        rook-version=v1.11.6
...
At this point, your Rook operator should be running version `rook/ceph:v1.11.6`.
