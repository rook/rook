...
example, when Rook v1.16.3 is released, the process of updating from v1.16.0 is as simple as running
the following:

```console
git clone --single-branch --depth=1 --branch v1.16.3 https://github.com/rook/rook.git
...
kubectl -n rook-ceph set image deploy/rook-ceph-operator rook-ceph-operator=rook/ceph:v1.16.3
```
...
the version `v1.16.3`. This upgrade should work from any official patch release of Rook v1.15 to any
...
```console
kubectl -n $ROOK_OPERATOR_NAMESPACE set image deploy/rook-ceph-operator rook-ceph-operator=rook/ceph:v1.16.3
```
...
availability and `rook-version=v1.16.3`, the Ceph cluster's core components are fully updated.

```console
Every 2.0s: kubectl -n rook-ceph get deployment -o j...

rook-ceph-mgr-a         req/upd/avl: 1/1/1      rook-version=v1.16.3
rook-ceph-mon-a         req/upd/avl: 1/1/1      rook-version=v1.16.3
rook-ceph-mon-b         req/upd/avl: 1/1/1      rook-version=v1.16.3
rook-ceph-mon-c         req/upd/avl: 1/1/1      rook-version=v1.16.3
rook-ceph-osd-0         req/upd/avl: 1//        rook-version=v1.16.3
...
At this point, the Rook operator should be running version `rook/ceph:v1.16.3`.
