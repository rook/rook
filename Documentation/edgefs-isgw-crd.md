...
- `direction`: I/O flow direction: `send`, `recieve` or `send+receive` (default).
...
- `useEncryptedTunnel`: If true, enables usage of encrypted SSH tunnel. By default, transfered metadata and data chunks are just compressed but unencrypted. Enable encryption if ISGW links crossing boundaries of single data center or multiple clouds.
