...
In addition to `cephConfig`, you can provide Ceph configuration values via Kubernetes Secrets using `cephConfigFromSecret`. This is useful for referencing sensitive values such as passwords or tokens that shouldn't be stored directly in the CR.
...
!!! warning
    If a value from `cephConfigFromSecret` cannot be retrieved — for example, if the referenced Secret or key is missing — Rook will log the issue and continue reconciling without applying that specific configuration entry.
