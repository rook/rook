...
## How to set a StorageClass to consume CephFilesystemSubVolumeGroup
...
* Extract the `clusterID` from the status of the CephFilesystemSubvolumegroup CR:
    ...
    ```bash
    kubectl -n rook-ceph get cephfilesystemsubvolumegroup -o yaml
    ```
...
* Set the `clusterID` in the storage class to this value instead of the name of the cluster namespace
