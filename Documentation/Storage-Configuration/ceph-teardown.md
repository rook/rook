...
Starting in v1.20, the CSI operator is always enabled. The `CephConnection` and `ClientProfile`
resources Rook creates are cleaned up automatically via ownerRefs when the `CephCluster` is deleted.
To be thorough in case any of these resources are left over, the commands below remove all four
CSI operator resource types:
