...
This section describes how to configure multiple `CephObjecStore` backed by the same storage pools.
...
- `protocols` to host `S3`, `Swift`, and `Admin-ops` APIs on a separate `CephObjecStore`-s.
