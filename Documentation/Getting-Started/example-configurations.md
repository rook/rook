...
* [`csi-operator.yaml`](https://github.com/rook/rook/blob/master/deploy/examples/csi-operator.yaml): CRDs and custom resources for the ceph-csi-operator. The CSI operator RBAC and Deployment are included in `common.yaml` and `operator.yaml` respectively.
