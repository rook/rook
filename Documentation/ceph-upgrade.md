...
another are as simple as updating the image of the Rook operator. For example, when Rook v1.1.4 is
released, the process of updating from v1.1.0 is as simple as running the following:
```
kubectl -n rook-ceph set image deploy/rook-ceph-operator rook-ceph-operator=rook/ceph:v1.1.4
```
