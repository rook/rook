...
```sh
git clone --single-branch --depth=1 --branch v1.8.9 https://github.com/rook/rook.git
...
the version `v1.8.9`. This upgrade should work from any official patch release of Rook v1.7 to any
...
```sh
kubectl -n $ROOK_OPERATOR_NAMESPACE set image deploy/rook-ceph-operator rook-ceph-operator=rook/ceph:v1.8.9
```
...
availability and `rook-version=v1.8.9`, the Ceph cluster's core components are fully updated.
...
At this point, your Rook operator should be running version `rook/ceph:v1.8.9`.
