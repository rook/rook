...
- `zapDevices`: Attempt to re-initialize previously selected `devices`. By default cluster assumes that selected devices have no logical partitions and considered empty. Only applicable when used with `dataDirHostPath`.
- `resurrect`: Attempt to recreate cluster based on previous CRD definition. If this flag set to true, then operator will only adjust networking. Often used together with `zapDevices` when clean up of old devices is needed. Only applicable when used with `dataDirHostPath`.
